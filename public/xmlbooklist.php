...
#<html>
#  <head>
#    <title>Booklist: xml parse example</title>
#  </head>
#  <body>
...
#
  ...
  class Booklist{
    ...
    function Booklist($filename){
      ...
      if($debug['methods']){
        $thisFunction ="Booklist::Booklist($filename)";
      ...
      function start_element($p,$element,$attributes){
        ...
        if($debug['methods']){
          $thisFunction ="Booklist::start_element(".$p.", ".$element.", attributes)";
          #echo_functionname($thisFunction);
        }
      ...
      function end_element($p,$element){
        ...
        if($debug['methods']){
          $thisFunction ="Booklist::end_element(".$p.", ".$element.")";
          #echo_functionname($thisFunction);
        }
      ...
      function cdata($p,$text){
        ...
        if($debug['methods']){
          $thisFunction ="Booklist::cdata(".$p.", ".$text.")";
          #echo_functionname($thisFunction);
        }
      ...
      function verify_order($p){
        ...
        if($debug['methods']){
          $thisFunction ="Booklist::verify_order($p)";
          echo_functionname($thisFunction);
	}
    ...
    function process_order($p){
        ...
        if($debug['methods']){
          $thisFunction ="Booklist::process_order(POST)";
          echo_functionname($thisFunction);
        }
...
#      }
  ...
  return true;
    ...
    function pre_process_order($p){
      ...
      global $paypal_info;
        ...
        if($debug['methods']){
          $thisFunction ="Booklist::pre_process_order(POST)";
          echo_functionname($thisFunction);
        }
        ...
        foreach ($this->records as $book){
          ...
          $itemnamecheck=str_replace(" ","_", "quantity_".$book['keyname']);
          $itemname=str_replace(".","_", $itemnamecheck);
          $prices[$itemname]=array();
          $prices[$itemname]['keyname']=$book['keyname'];
          $prices[$itemname]['price']=$book['price'];
          $i++;
          }
  ...
  function order_list(){
      ...
      if($debug['methods']){
        $thisFunction ="Booklist::order_list()";
        echo_functionname($thisFunction);
      }
...
#      
	  ...
	  printf("<a href=\"%s\">
		  <img src=\"%s\" height=\"180\" alt=\"%s\" border=\"0\"/>
                    ...
                    $_SERVER['PHP_SELF'].'?pageid='. $book['pageid'],
                    $book['image'],
                    $book['title']
		  );
	  ...
	  printf("<p class=\"details\">%s %s + postage and packing</p><br/>\n",
          ...
          echo "<select name=\"quantity_".$book['keyname']."\">\n";
          ...
          echo "</select>&nbsp;";
	        ...
	        echo "<label for=\"quantity_".$book['keyname']."\">Select number of copies required</label>\n";
          echo "<br/>\n";
    ...
    function show_menu(){
      ...
      if($debug['methods']){
        $thisFunction ="Booklist::show_menu()";
        echo_functionname($thisFunction);
      }
	  ...
	  printf("<p>%s</p><br/> <p class=\"details\">%s %s + postage and packing</p> <p class=\"details\">ISMN:%s</p><p class=\"details\">Spartan Press Reference: <a href=\"http://www.spartanpress.co.uk/spweb/details.php?catno=%s\">%s</a></p> </td>\n",
                    #$authors,
                    $book['subtitle'],
                    "£",
                    $book['price'],
          ...
          #echo "</tr>\n";
    #printf("<td align=\"left\" valign=\"center\"><img src=\"%s\" alt=\"%s\" height=\"180\"></td>\n",$book['imagecenter'],$book['title']);
#
          echo "<td>\n";
          #echo "<div class=\"PayPalPayNow\">\n";
          #echo "<h5>Don't Buy now with PayPal - instead visit Spartan Press where you can purchase this item directly</h5>\n";
          printf("<p class=\"spartan\"> <a href=\"http://www.spartanpress.co.uk/spweb/details.php?catno=%s\">
                  Purchase now from Spartan Press<br/>Reference: %s</a> </p> </td>\n",
                    $book['reference'],
                    $book['reference']
	        );
          ...
          #echo "<input type=\"hidden\" name=\"item_name\" value=\"".$book['keyname'].", ISMN:".$book['ismn']."\">\n";
	        #echo "<input type=\"hidden\" name=\"amount\" value=\"".$book['price']."\">\n";
          #echo "<br/>\n";
          ...
          #echo "</form></br>\n";
          #echo "<p>&pound;".$book['price']." plus P+P</p>\n";
          #echo "</div>\n";
          echo "</td>\n";
          echo "</tr>\n";
	  }
      ...
      if($debug['methods']){
        $thisFunction ="Booklist::show_book($pageid)";
        echo_functionname($thisFunction);
      }

        foreach($this->records as $book){
          if($book['pageid'] !== $pageid){
            continue;
          }

          $authors = join(', ', $book['author']);
          $categories = join(', ', $book['category']);
          ...
          $zoom_more=array(
            'ISMN: '.$book['ismn'],
            'Illustration by '.$book['illustration'],
            'Sample page '.$authors
            );
 ...
 #           printf("<img src=\"%s\" onClick=\"javascript: pop_zoom('%s','%s','%s',680,900);return false;\" alt=\"%s\" height=\"240\"/>\n<br/><p>%s</p></a>\n", $zoom_data[$i], $zoom_data[$i], $book['title'], $zoom_more[$i], $book['title'], $zoom_more[$i]);
          ...
          echo "</tr></table>\n<br/>\n";
	        ...
	        printf("<p class=\"spartan\"><a href=\"http://www.spartanpress.co.uk/spweb/details.php?catno=%s\">Purchase now from Spartan Press<br/>Reference %s</a></p>\n",
                    $book['reference'],
                    $book['reference']
	        );
          echo "<div class=\"PayPalPayNow\">\n";
