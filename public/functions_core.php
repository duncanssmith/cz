...
function generate_menu($p){
	...
	if($debug['functions']){
	 ...
	 $filename=$paths['include']."/".$files['menuintermed'];
	...
	#echo "</pre></p>\n";
		...
		$menufile=$paths['include']."/".$files['menu'];
...
function showpage($p,$cols){
  ...
  if($settings['php5']){
    $my_library = new Booklist($xmlfile);
      ...
      ($_GET['pageid']!=='2.0'))
    {
      //return info on one book
        $my_library->show_book($_GET['pageid']);
    }else{
      //show menu of books
      if(($_GET['pageid']==='2')){
	      ...
	      if((isset($_GET['param']))&&($_GET['param']==true)){
		    ...
		    # we have an order in the SESSION
		    # and a set of customerdetails 
		    # in the POST
		    $my_library->verify_order($tables[0]);
		...
		$my_library->get_customer_details($tables[0]);
	      }else{

		if(isset($_POST['submit'])){
		  if($_POST['submit']=='proceed'){
		    $my_library->pre_process_order($_POST);
		  }elseif($_POST['submit']=='confirm'){
		    $my_library->process_order($_POST);
		  }
		}else{
		  $my_library->order_list();
		}
	      }
      }else{
        $my_library->show_menu();
